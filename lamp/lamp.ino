...
const String serverIp = "192.168.1.32:8080";
void sendPredictRequest(String ext) {
  const String serverUrl = "http://" + serverIp + "/" + ext;
  if ((WiFi.status() == WL_CONNECTED)) {
    HTTPClient http;
    http.begin(serverUrl);
    int httpCode = http.GET();  // Send HTTP GET
...
void loop() {
  ...
  if (!btnState){
    ...
    if (alrPr){
      alrPr = 0;
      unsigned long pr = millis() - pressTime;
      Serial.println(pr);
      if (pr >= 1000) sendPredictRequest("summary");
      else sendPredictRequest("predict");
    }
  }
  delay(10);
}
